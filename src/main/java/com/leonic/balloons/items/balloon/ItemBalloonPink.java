	...
	public boolean onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ)
	{
		...
		if(!worldIn.isRemote){
			...
			if(playerIn.canPlayerEdit(pos1, side, stack) && playerIn.canPlayerEdit(pos2, side, stack)){
				worldIn.setBlockState(pos1, BalloonsBlocks.blockBalloonPink.getDefaultState());
				worldIn.setBlockState(pos2, BalloonsBlocks.blockBalloonPinkColls.getDefaultState());
				worldIn.notifyBlockOfStateChange(pos1, BalloonsBlocks.blockBalloonPink);
				worldIn.notifyBlockOfStateChange(pos2, BalloonsBlocks.blockBalloonPinkColls);
				--stack.stackSize;
				return true;
			}else{
				return false;
