...
public class ItemBalloonRed extends Item {
	...
	public ItemBalloonRed(String name){
		...
		setTextureName(References.MODID + ":" + name + "_red");
	...
	public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int var7, float var8, float var9, float var10)
	{
		if(!world.isRemote){
			...
			if(player.canPlayerEdit(x, y + 1, z, var7, stack) && player.canPlayerEdit(x, y + 2, z, var7, stack)){
				world.setBlock(x, y + 1, z, BalloonsBlocks.blockBalloonRed);
				world.setBlock(x, y + 2, z, BalloonsBlocks.blockBalloonRedColls);
				world.notifyBlockOfNeighborChange(x, y + 1, z, BalloonsBlocks.blockBalloonRed);
				world.notifyBlockOfNeighborChange(x, y + 2, z, BalloonsBlocks.blockBalloonRedColls);
				--stack.stackSize;
				return true;
			}else{
				return false;
