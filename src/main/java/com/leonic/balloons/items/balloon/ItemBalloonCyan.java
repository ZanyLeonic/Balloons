	...
	public boolean onItemUse(ItemStack stack, EntityPlayer playerIn, World worldIn, BlockPos pos, EnumFacing side, float hitX, float hitY, float hitZ)
	{
		...
		if(!worldIn.isRemote){
			...
			if(playerIn.canPlayerEdit(pos1, side, stack) && playerIn.canPlayerEdit(pos2, side, stack)){
				worldIn.setBlockState(pos1, BalloonsBlocks.blockBalloonCyan.getDefaultState());
				worldIn.setBlockState(pos2, BalloonsBlocks.blockBalloonCyanColls.getDefaultState());
				worldIn.notifyBlockOfStateChange(pos1, BalloonsBlocks.blockBalloonCyan);
				worldIn.notifyBlockOfStateChange(pos2, BalloonsBlocks.blockBalloonCyanColls);
				--stack.stackSize;
				return true;
			}else{
				return false;
